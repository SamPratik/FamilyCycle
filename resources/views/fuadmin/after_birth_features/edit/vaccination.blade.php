...
@section('content')
    <form class="" action="{{ route('fuadmin.update.vaccination', $vaccination->id) }}" method="post">
        {{ csrf_field() }}
        <input type="hidden" name="_method" value="PUT">
        <div class="form-group">
          <label for="comment">Edit:</label>
          <textarea name="content" class="form-control" rows="50" id="comment">{!! $vaccination->content !!}</textarea>
        </div>
        <div class="form-group">
